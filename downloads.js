...
var kdown = {
   ...
   marketDD : {
      ...
      bind : function () {
         kdown.router.add("market_change",function () {
            kdown.db.market = $(kdown.db.MARKETDD).val();
            kdown.hash.update();
            kdown.db.ui_update();
         });

         $(kdown.db.MARKETDD).change(function () {
            kdown.router.fire("market_change");
         });
      },
   },
   langDD : {
      ...
      bind : function () {
         $(kdown.db.LANGDD).change(function () {
            kdown.router.fire("table_filter");
         });
      }
   },
   catList : {
      ...
      load : function () {
         ...
         $.each(cats, function(code, cat) {
            ...
            var hash = "#" + encodeURIComponent(cat) + db.market;
         ...
         for (var i = 0, l = json.markets.length; i < l; i ++) {
            ...
            for( var cat in json.cats ) {
               if (json.cats.hasOwnProperty(cat)) {
                  kdown.router.add("#" + cat + "@" + market,
                                   kdown.db.changer(cat, market));
               }
            }
         }
